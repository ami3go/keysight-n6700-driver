...
- Read-only checks, the N679xA electronic-load write path (priority mode,
  level setpoints, input on), and the standalone *guarded output test*
  (`test_guarded_output_enable_measure_disable`) have all now been run
  against real hardware — see [`review/known_risks.md`](../review/known_risks.md)
  for what's still simulator-only (a genuine SMU module, protection
  trip/clear, remote/local control). Treat any first real-hardware
  energizing run of your own as a new-instrument acceptance test: verify
  wiring, polarity, and DUT limits, and keep an operator present for first
  energization.
